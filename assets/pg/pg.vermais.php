...
<?php
		...
		$nome_room = $class->nome_room($_GET['pga']);
		$info_img = $class->room_imagens($_GET['pga']);
		$text_room = $class->room_texto($_GET['pga']);
		$valor_room = $class->valor_room($_GET['pga'],  date('Ymd'));
		$i = 0;
		if($nome_room && $info_img){
				$html .= "<div class='InforVerMaisPersonalizeCarroselImg owl-carousel owl-theme' id='".$_GET['pga']."'>";
					$i = 0;
				...
				$html .= "<script> $(document).ready(function(){ $('#".$_GET['pga']."').owlCarousel({items: 1, lazyLoad: true, lazyLoadEager: 1, loop: true, margin: 10, autoHeight: true, autoplay:true, autoplayTimeout:20000,}); });</script> </li>";
		}

	echo $html;
	$info = $class->valores_rooms($_GET['pga']);
  	...
  	<div class="informacoesDate">
		...
		<div class="RowButton" style="width: 50px;margin: 0 auto;margin-right: 55%;margin-top: 2rem;">
			<div class="RowButtonConteiner">
				<a href="javascript:verificar_room('<?=$_GET['pga'];?>');">
